  ...
  if( unit ){
    ...
    if( (resp->sensor1 != sresp.sensor1) ){
			if( resp->sensor1 ){
				...
				syslog( LOG_NOTICE, "%s : Sensor1 ALARM CANSEL!",m->get_chname());
			}
		}
    if( (resp->sensor2 != sresp.sensor2) ){
			if( resp->sensor2 ){
				...
				syslog( LOG_NOTICE, "%s : Sensor2 ALARM CANSEL!",m->get_chname());
			}
		}
    if( (resp->sensor3 != sresp.sensor3) ){
			if( resp->sensor3 ){
				...
				syslog( LOG_NOTICE, "%s : Sensor3 ALARM CANSEL!",m->get_chname());
			}
		}
