...
int jason_sensors(int indent, app_comm_cli &cli, char *chan, unit u) {
	...
	if (!resp->frame_ptr()) {
		char err_str[256];
		sprintf(err_str,
			"Error: bad message from eocd for sensors of unit: %d", u);
		jason_error(err_str);
		ret = -1;
		goto err_exit;
	}
	...
	if (resp->is_negative()) { // no such unit or no net_side
		char err_str[256];
		sprintf(err_str,
			"Error: negative response from server for sensors of unit: %d", u);
		jason_error(err_str);
		ret = -1;
		goto err_exit;
	}
...
int jason_channel(int indent, app_comm_cli &cli, char *chan,
...
int jason_m15ints(int indent, app_comm_cli &cli, char *chan, unit u, side s,
	int loop, int inum) {
	char *b;
	int size;
	int offset = 0;
	...
	for (inum = start_int; inum <= end_int; inum++) {
		fr = new app_frame(APP_ENDP_15MIN, APP_GET_NEXT, app_frame::REQUEST, 1,
			chan);
		...
		if (ret = fr1->is_negative()) { // no such unit or no net_side
			...
			if (p1->unit != p->unit || p1->side != p->side || p1->loop
				!= p->loop) {
				char str[256];
				sprintf(
					str,
					"15-minutes int: No information about unit=%d, int=%d aviliable",
					u, inum);
				jason_error(str);
				ret = -1;
				break;
...
int jason_d1ints(int indent, app_comm_cli &cli, char *chan, unit u, side s,
	int loop, int inum) {
	char *b;
	int size;
	int offset = 0;
	...
	for (inum = start_int; inum <= end_int; inum++) {
		fr = new app_frame(APP_ENDP_1DAY, APP_GET_NEXT, app_frame::REQUEST, 1,
			chan);
		...
		if (ret = fr1->is_negative()) { // no such unit or no net_side
			...
			if (p1->unit != p->unit || p1->side != p->side || p1->loop
				...
				sprintf(
					str,
					"1-day int: No information about unit=%d, int=%d aviliable",
					u, inum);
				jason_error(str);
				ret = -1;
				break;
...
int jason_loop(int indent, app_comm_cli &cli, char *chan, unit u, side s,
	int loop) {
	char *b;
	int size;
	char date[256];
...
int jason_side(int indent, app_comm_cli &cli, char *chan,
	span_params_payload *p, unit u, side s) {
	int ret = 0;
...
int jason_unit(int indent, app_comm_cli &cli, char *chan,
	span_params_payload *p, unit u) {
	int ret = 0;
	int offset = 0;
...
int jason_exact(int indent, app_comm_cli &cli, char *chan, unit _unit) {
	app_frame *req = new app_frame(APP_SPAN_PARAMS, APP_GET,
		app_frame::REQUEST, 1, chan);
	app_frame *resp;
	char *buf;
	int ret = 0;
	int offset = 0;
