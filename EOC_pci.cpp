...
int pci_iflist(DIR *dir, char *dpath, char *pdev,
	char iflist[MOD_MAX_DEVS][IF_NAME_LEN]) {
	struct dirent *ent;
...
pci2dname(int pcislot, int pcidev) {
	...
	PDEBUG(DERR, "Start");snprintf(drvpath,PATH_SIZE,DEVICE_PATH "/0000:00:%02d.%d/driver",pcislot,pcidev);
	...
	PDEBUG(DERR, "Reading result end");

	// ----------------------

	switch (type) {
	case 0:
		PDEBUG(DERR, "Return name=%s", ifaces[0]);
		return strdup(ifaces[0]);
	case 1:
		if (pcidev >= ifcnt) {
			PDEBUG(DERR, "Return name=NULL");
			return NULL;
		}
		PDEBUG(DERR, "Return name=%s", ifaces[pcidev]);
